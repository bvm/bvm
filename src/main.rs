...
async fn handle_install_command<TEnvironment: Environment>(
    ...
    if command.use_command {
        for entry in config_file.binaries.iter() {
            ...
            let command_name = binary.get_command_name();
...
async fn handle_install_url_command<TEnvironment: Environment>(
    ...
    if is_installed && !command.force {
        ...
        let was_global_version = if let Some(identifier) = identifier {
    ...
    if command.use_command {
        let identifier = plugin_manifest
            ...
            .map(|identifier| identifier.clone())
            .unwrap();
        let command_name = plugin_manifest.get_binary(&identifier).unwrap().get_command_name();
        plugin_manifest.use_global_version(command_name, plugins::GlobalBinaryLocation::Bvm(identifier));
    }
...
fn set_global_binary_if_not_set(
    ...
    Ok(if plugin_manifest.get_global_binary_location(&command_name).is_none() {
        ...
        plugin_manifest.is_global_version(&identifier)
    })
}
...
mod test {
    ...
    async fn install_url_command_previous_install() {
        ...
        create_remote_zip_package(&environment, "http://localhost/package4.json", "owner", "name", "4.0.0");
        ...
        assert_eq!(
            ...
            vec![
                ...
                "Installed. Run `bvm use name 4.0.0` to use it on the path as 'name'."
            ]
        );
        assert_resolves!(&environment, third_binary_path);
