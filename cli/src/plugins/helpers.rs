    ...
    Some(location) => match location {
      ...
      GlobalBinaryLocation::Bvm(identifier) => {
        if let Some(item) = plugin_manifest.get_binary(&identifier) {
          let command_exe_path =
            get_exec_binary_command_exe_path(environment, &item, command_name).expect("Expected to have a command.");
          Ok(command_exe_path)
        } else {
...
pub fn has_command_name_for_exec<TEnvironment: Environment>(
  ...
  Ok(match &version_selector {
    PathOrVersionSelector::Path => {
      let command_names =
        get_command_names_for_name_and_path_or_version_selector(plugin_manifest, name_selector, version_selector)?;
